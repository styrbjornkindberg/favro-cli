...
That topic carries the whole contract: the mandatory scope lock, the seven
intents and their CLI spellings, the two relationships (and the unordered one
that does not exist), the retry contract, and — up front — what a failed
multi-step write may leave behind.
