 ...
 * where the table above comes from), and for `skill edit` it BLOCKS — so a
 ...
 * from `interactive-commands.ts`, `refuses ... without spawning` timed out at
 * the 4s bound (vs 0.4s green) and its output carried `CHILD-SPAWNED-BLOCKING`.
 *
...
beforeAll(() => {
  ...
  // The child. Reports its inherited fds, then blocks for `skill edit` — the
  // blocking is what makes a removed guard fail instead of pass. `sleep`, not an
  // unbounded read: if SIGKILL ever misses it, it reaps itself.
  ...
  writeFileSync(
    ...
    [
      ...
      'case "$1 $2" in',
      '  "skill edit") echo CHILD-SPAWNED-BLOCKING; sleep 30 ;;',
      '  *) echo "CHILD-RAN $*" ;;',
      'esac',
      '',
    ].join('\n'),
...
describePty('favro shell under a real pty', () => {
  ...
  test('refuses skill edit without spawning, rather than handing vi a pipe', () => {
    ...
    expect(run.timedOut).toBe(false);
    expect(run.out).not.toContain('CHILD-SPAWNED-BLOCKING');
    expect(run.out).not.toContain('CHILD-FDS');
